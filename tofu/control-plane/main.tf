...
resource "cloudflare_workers_script" "scheduled_teardown" {
  ...
  plain_text_binding {
    ...
    text = "48 20 * * *"  # Notification at 20:45 UTC (21:45 CET)
  }

  plain_text_binding {
    name = "TEARDOWN_CRON"
    text = "3 21 * * *"  # Teardown at 21:00 UTC (22:00 CET)
  }
...
resource "cloudflare_workers_cron_trigger" "scheduled_teardown" {
  ...
  schedules   = [
    "48 20 * * *",  # Notification at 20:45 UTC (21:45 CET)
    "3 21 * * *",   # Teardown at 21:00 UTC (22:00 CET)
  ]
}
...
resource "cloudflare_pages_domain" "control_plane" {
  ...
  domain       = "control-fabrice-bertschi.nona.company"
...
resource "cloudflare_zero_trust_access_application" "control_plane" {
  ...
  cors_headers {
    allowed_origins   = ["https://control-fabrice-bertschi.nona.company"]
