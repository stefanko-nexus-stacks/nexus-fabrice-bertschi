...
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "main" {
  ...
  config {
    # SSH access
    ingress_rule {
      hostname = "ssh-fabrice-bertschi.nona.company"
    ...
    dynamic "ingress_rule" {
      ...
      content {
        hostname = "${ingress_rule.value.subdomain}-fabrice-bertschi.nona.company"
...
resource "cloudflare_record" "services" {
  ...
  zone_id = var.cloudflare_zone_id
  name    = "${each.value.subdomain}-fabrice-bertschi"
  content = "${cloudflare_zero_trust_tunnel_cloudflared.main.id}.cfargotunnel.com"
  type    = "CNAME"
  proxied = true
  ttl     = 1
...
resource "cloudflare_record" "firewall_tcp" {
  ...
  zone_id = var.cloudflare_zone_id
  name    = "${each.value.dns_record}-fabrice-bertschi"
...
resource "cloudflare_zero_trust_access_application" "ssh" {
  ...
  domain           = "ssh-fabrice-bertschi.nona.company"
  type             = "ssh"
  session_duration = "1h"
}
...
resource "cloudflare_zero_trust_access_application" "services" {
  ...
  domain            = "${each.value.subdomain}-fabrice-bertschi.nona.company"
  type              = "self_hosted"
