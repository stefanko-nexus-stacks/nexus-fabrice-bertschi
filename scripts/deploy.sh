...
SSH_HOST="ssh-fabrice-bertschi.nona.company"
...
KESTRA_URL=https://kestra-fabrice-bertschi.nona.company
EOF
...
CB_SERVER_URL=https://cloudbeaver-fabrice-bertschi.nona.company
...
REDIRECT_URL=https://hoppscotch-fabrice-bertschi.nona.company
WHITELISTED_ORIGINS=https://hoppscotch-fabrice-bertschi.nona.company
VITE_BASE_URL=https://hoppscotch-fabrice-bertschi.nona.company
VITE_SHORTCODE_BASE_URL=https://hoppscotch-fabrice-bertschi.nona.company
VITE_ADMIN_URL=https://hoppscotch-fabrice-bertschi.nona.company/admin
VITE_BACKEND_GQL_URL=https://hoppscotch-fabrice-bertschi.nona.company/backend/graphql
VITE_BACKEND_WS_URL=wss://hoppscotch-fabrice-bertschi.nona.company/backend/graphql
VITE_BACKEND_API_URL=https://hoppscotch-fabrice-bertschi.nona.company/backend/v1
...
PREFECT_UI_API_URL=https://prefect-fabrice-bertschi.nona.company/api
EOF
...
LAKEFS_GATEWAYS_S3_DOMAIN_NAME=s3.lakefs-fabrice-bertschi.nona.company
...
NC_PUBLIC_URL=https://nocodb-fabrice-bertschi.nona.company
                    ...
                    SECRETS_PAYLOAD=$(cat <<SECRETS_EOF
{
  ...
  "secrets": [
    ...
    {"secretKey": "GITEA_REPO_URL", "secretValue": "https://git-fabrice-bertschi.nona.company/${ADMIN_USERNAME}/nexus-${DOMAIN//./-}-gitea.git", "tagIds": ["$GITEA_TAG"]},
    ...
    cron: \"*/15 * * * *\"'" >/dev/null 2>&1 || true
            ...
            if echo "$ENABLED_SERVICES" | grep -qw "woodpecker" && [ -n "$WOODPECKER_AGENT_SECRET" ]; then
                ...
                OAUTH_RESULT=$(ssh nexus "curl -s -X POST 'http://localhost:3200/api/v1/user/applications/oauth2' \
                    ...
                    -d '{
                        \"name\": \"Woodpecker CI\",
                        \"redirect_uris\": [\"https://woodpecker-fabrice-bertschi.nona.company/authorize\"],
...
if echo "$ENABLED_SERVICES" | grep -qw "wikijs" && [ -n "$WIKIJS_ADMIN_PASS" ]; then
    (
        ...
        SETUP_PAYLOAD=$(jq -n \
            ...
            --arg url "https://wiki-fabrice-bertschi.nona.company" \
